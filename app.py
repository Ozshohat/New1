...
def clean_and_prepare_df(df_raw):
    # חיפוש שורת header אמיתית – מחפש את השורה הראשונה שיש בה מילות מפתח לזיהוי תאריך
    header_row = None
    for i, row in df_raw.iterrows():
        row_str = row.astype(str).str.cat(sep=' ')
        if any(keyword in row_str for keyword in ["תאריך", "תאריך עסקה", "ת. עסקה", "תאריך חיוב", "Transaction Date", "Date"]):
            header_row = i
            break
    ...
    # קריאה מחדש תוך דילוג על שורות לפני שורת הכותרת
